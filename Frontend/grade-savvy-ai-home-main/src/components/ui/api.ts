...
// ── New type for per-question breakdown ───────────────────────────────────────
export interface PerQuestionResult {
  question_num: number;
  question: string;
  student_answer_excerpt: string;
  question_type?: string;           // factual | conceptual | analytical | numerical | code | list
  routed_score_detail?: Record<string, unknown>;
  used_groq_fallback?: boolean;
  score?: number;
...
export interface StudentResult {
  ...
  // ── NEW: per-question aligned results ────────────────────────────────────
